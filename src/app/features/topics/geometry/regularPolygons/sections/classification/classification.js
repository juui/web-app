...
class Classification {
  ...
  constructor($timeout) {
    ...
    let snapClassification = Snap('#svg-regularPolygons');

    const options = paths[this.sides];
    this._polygon = snapClassification.path(options.d);
    this._polygon.attr({
      fill: options.fill,
      stroke: '#000',
      strokeWidth: 1,
  ...
  onChangeSides(sides) {
    this._untouch = false;
    this.animateClassification(sides);
  }
